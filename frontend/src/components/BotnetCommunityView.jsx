...
export default function BotnetCommunityView() {
  ...
  const allNodes = [
    {
      ...
      community: 1,
      degree: 14,
      role: "Command & Control (C2) Server",
    },
    ...botNodes,
    ...benignNodes,
  ];
  ...
  const activeNodeInfo =
    allNodes.find((n) => n.id === selectedNode) || allNodes[0];
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-2xl">
              Identifies distributed botnet swarms and coordinated
              command-and-control infrastructures using Clauset-Newman-Moore
              Greedy Modularity Optimization on network traffic flow graphs.
            </p>
          </div>
      ...
      {viewMode === "interactive" ? (
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Click any node to inspect connected flow edges and community
                  clustering
                </p>
              </div>
                ...
                {(filterCluster === "all" || filterCluster === "botnet") && (
                  <g stroke="#b91c1c" strokeWidth="1.2" opacity="0.6">
                    {botNodes.map((bot, i) => {
                      ...
                      const isHighlighted =
                        selectedNode === c2Server || selectedNode === bot.id;
                      return (
                        <line
                          key={bot.id}
                          x1="260"
                ...
                {(filterCluster === "all" || filterCluster === "benign") && (
                  <g stroke="#15803d" strokeWidth="1" opacity="0.5">
                    {benignNodes.map((b, i) => {
                      ...
                      const nextAngle =
                        ((i + 1) / benignNodes.length) * 2 * Math.PI;
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-5 shadow-xs text-xs space-y-4">
            ...
            <div>
              ...
              <span className="font-semibold text-slate-800 dark:text-slate-200">
                Community #{activeNodeInfo.community} (
                {activeNodeInfo.community === 1
                  ? "Botnet Star Swarm"
                  : "Internal LAN"}
                )
              </span>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-5 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                Generated Topology Plot (
                <span className="font-mono">botnet_topology.png</span>)
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Rendered artifact matching{" "}
                <span className="font-mono">
                  streaming/botnet_community_detection.py
                </span>{" "}
                (300 DPI Matplotlib export)
              </p>
            </div>
            <a

...
export default function NoSqlServingView({ baselines, liveAlerts }) {
  ...
  const sampleBlacklist = (baselines || [])
    .filter((b) => b.count > 5)
    .slice(0, 50)
    .map((b, i) => ({
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-5 shadow-xs transition-colors">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
          Test any IP against the NoSQL MongoDB Blacklist collection to verify
          indexed query response time vs unindexed full table scan.
        </p>
          ...
          <div className="mt-4 p-4 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 text-xs">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-slate-200 dark:border-slate-700 pb-3 mb-3">
              <div>
                ...
                <span
                  className={`font-semibold font-mono text-sm ${queryResult.found ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400"}`}
                >
                  {queryResult.status}
                </span>
              </div>
            ...
            <div>
              <span className="text-slate-400 block text-[11px] mb-1 font-mono">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-xs overflow-hidden transition-colors">
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-slate-400">
            {activeCollection === "blacklist"
              ? `${sampleBlacklist.length} docs shown`
              : `${liveAlerts?.length || 0} events stored`}
          </span>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {activeCollection === "blacklist" ? (
                sampleBlacklist.map((doc, idx) => (
                  ...
                  >
                    <td className="py-2 px-4 text-slate-400 dark:text-slate-500">
                      {doc._id}
                    </td>
                ...
                <tr>
                  ...
                  >
                    No live alerts recorded yet. Inject a Blacklisted Attacker
                    flow or DDoS flood to trigger alerts.
                  </td>
                </tr>
              ) : (
